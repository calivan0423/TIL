...
#  zigbee IEEE802.15.4 Networking layers

     그림 3.1은 직비 무선 네트워킹 프로토콜계층을 보이고 있다.
     직비 프로토콜 계층은 iso의 osi 모델에 기초를 두고 있다.
     osi 에는 7계층이 있지만 직비에는 오직 저전력,저데이터율,무선네트워킹에 필수적인 계층만을 가지고 있다.
      PHY 와 MAC 계층은 IEEE 802.15.4표준에 의해 정의되고 NWK와 APL 계층은 직비 표준에 의해 정의된다.
      보안 기능은 양쪽 모두의 표준이다. 3.1 그림의 모든 네트워크 요소들은 직비 무선 통신을 위하여 고려되었다.
      
      각 계층과 인접한 계층간의 통신은 SAPs(service access points)를 통하여 이러어 진다.
      한 sap는 한 계층에서 다른 계층으로 서비스를 요청하는 개념적인 지점이다.
      
![image](https://user-images.githubusercontent.com/43804441/51961633-bade2a00-24a0-11e9-9d09-fc8deaa1252d.png)

      
# THE IEEE 802.15.4 PHY Specifications 

     802.15.4는 물리층의 프로토콜 기능과 MAC층과의 상호작용 뿐 아니라 수신기의 감도와 전송출력과 같은 하드웨어 레벨의 최저 요구사항도 정의한다.
     그러나 상용 송수신기는 IEEE 802.15.4의 최소 요건을 초과할 수 있다.
     
     
![image](https://user-images.githubusercontent.com/43804441/51961646-c9c4dc80-24a0-11e9-8fd3-924114de145c.png)
     
     
## CHANNEL ASSIGNMENT  채널 할당
  
   주파수 채널은 채널 번호와 채널 페이지의 조합으로 결된다.
   채널 페이지는 IEEE 802.15.4에서 지원되는 물리층들 사이를 구별하기 위해 추가된 개념이다.
   이 전의 802.15.4 표준에서는 주파수 채ㅓㄹ은 단순히 채널 번호에 의하여 구별되었고 다른 어떤 물리층의 옵션도 없었다.
   처음 릴리즈에서  총 27 채널 이상의 어떤 규정도 없었다. 그래서 다중 운영 주파수 대역을 구현하는 물리층들은 지원될 수 없었다.
   각 채널 페이지는 최대 27개의 채널을 가진다. 표 3.1은 802.15.4 표준의 채널 할당를 표여준다.
   채널 페이지 0-2는  현재 868/915MHz 와2.4GHz 대역에서 사용된다.
   채널 페이지 3-31은 미래의 잠재적 사용을 위하여 예약되었다.
    
   채널 페이지 0는 802.15.4의 2003 버전에 정의된 모든 채널을 지원한다.
   1번과 2번 채널 페이지들은 2006년판 표준에서 소개된 선택적인 물리층들에 의해 사용된다. 
   
   각 채널은 채널 번호로 구별된다. 처음 3개의 채널 페이지에서 채널 번호 0은 주파수의 중심이 868.3MHz로 868Mhz대역에 할당되었다.
   다른 주파수 대역의 경우 가장 낮은 채널 번호는 1로, 주파수가 가장 낮은 채널에 할당된다.
    
   915MHz, 2.4GHz 대역에 있는 채널들의 센터 주파수들은 밑에 방정식으로 계산할 수 있다.
...
## Summary of the PHY Layer Responsibilities
    ...
    물리 계층은 하드웨어와 가까운 계층이고 라디오 송수신기를 직접적으로 조작하고 통신합니다.
    물리계층의 책임은 다음과 같습니다.
    
    1. 라디오 송수신기의 활동/비활동
    2. 데이터의 송/수신
    3. 주파수 채널의 선택 ( 송수신기가 작동할 정확한 주파수)
    4. ED 수행  ED는 관심 주파수 대역 내에서 신호 에너지를 추정하는 작업입니다.
        이 추정치는 채널이 명확한지 여부와 전송에 사용할 수 있는지를 파악하는 데 사용됩니다.
    5. CCA 수행
    6. LQI 발생. LQI는 수신자가 수신하는 데이터 패킷의 품질을 나타냅니다. 
    신호 강도는 신호 품질의 표시로 사용할 수 있습니다
