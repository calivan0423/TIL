...
#### THE PAN ID conflict notifiation command format

  PAN 코디네이터가 자체 네트워크를 구성할 때, 먼저 채널 스캔을 수행하여 다른 가능한 네트워크를 찾을 것이다.
  PAN 코디네이터는 다른 인근 네트워크에서 사용 중이지 않은 PAN 식별자(PAN ID)를 선택할 것이다.
  그러나 자체 네트워크 구성 후에 코디네이터는 POS에 같은 PAN ID를 가진 다른 네트워크가 있다는 것을 발견할 수 있다.
  이러한 경우, PAN 코디네이터는 이 문제를 해결하기 위해서 PAN ID 충돌 해결 단계를 따를 것이다.
  
  PAN 코디네이터가 인근에 동일한 PAN ID를 가진 다른 네트워크가 존재를 인식하는 방법에는 두 가지가 있다.
  
  * PAN 코디네이터가 같은 PAN ID를 가진 다른 코디네이터로 부터 비콘 프레임을 수신한다.
    비콘 프레임에서, 만약 PAN코디네이터 부필드가 1으로 설정되었다면, 비콘은 PAN 코디네이터로부터 왔다는 것을 의미한다.
    
  * PAN 코디네이터와 연결된 장비는 동일한 PAN ID를 가진 다른 코디네이터의 존재를 확인하고, PAN ID 충돌 알림 명령을 사용하여
    충돌에 대해 자체 코디네이터에게 알린다.
    장비는 장비가 연결된 PAN과 동일한 PAN ID를 가진 PAN 코디네이터로부터 비콘 프레임을 수신한다면, PAN ID가 충돌하지만,
    코디네이터 주소가 장비가 가진 MAC-PIB에 있는 주소와 일치하지 않는다고 결론 짓는다.
    PAN ID 충돌 알림을 전송한 후에, 장비가 PAN 코디네이터로부터 승인을 수신한다면, 장비 MLME는 MLME-SYNC-LOSS.indication 기본요소를 사용하여
    자신의 NKW층에게 PAN ID 충돌 때문에 동기화가 손실되었다고 알릴 것이다.
    
  PAN 코디네이터가 이 방법을 통해서 PAN ID 충돌을 탐지한 후에, PAN 코디네이터는 능동 검색을 수행하여
  기존의 PAN ID들을 확인하고 새롭고 유일한 PAN ID를 선택할 수 있다.
  그런 다음 PAN 코디네이터는 이에 따라 슈퍼프레임 구성을 업데이트 할 것이다.
  
  
#### THE Orpahn notification and beacon request commnad format    

  고아 알림 명령의 MAC 페이로드는 오직 명령 프레임 식별자만 포함하고 고아 알림 절차의 부분으로 사용 된다.
  
  비콘 요청 명령은 RFD에 대해  옵션이고, 장비의 POS에서 모든 코디네이터를 찾는데 사용된다.
  MAC 페이로드는 오직 명령 프레임 식별자만을 포함한다.


![image](https://user-images.githubusercontent.com/43804441/52619089-1f0ede00-2ee4-11e9-83c6-e6fb81040584.png)


#### THE coordinator realignment and GTS request command formats

  코디네이터는 네트워크 설정의 수정이 필요한 네트워크 변경이 있을 때마다 재배치 명령을 사용할 것이다.
  예를 들어, 만약 PAN ID 충돌이 있을 경우, PAN 코디네이터는 새롭고 유일한 PAN ID를 선택할 것이다.
  이 경우 PAN 코디네이터는 새로운 PAN ID를 해당 네트워크의 장비에게 전달하기 위하여 재배치 명령을 사용한다.
  
  GTS 할당과 할당 해제는 GTS 요청 명령을 사용하여 수행된다. (그림 3.28b)
  이 명령은 RFD, FFD 둘다에서 선택 사항이다.
  GTS 직접 필드는 수신전용 혹은 전송전용모드로 설정될 수 있다. 만약 특성 타입이 1로 설정되면, 이 명령은 GTS를 할당하는데 사용된다.
  만약 0으로 설정되면, 이 명령은 GTS 할당 해제에 사용된다.
  
  
#### THE MAC promiscuous mode of operation

  장비가 비 규칙적인 작동 모드에 있을 때, 수신자는 수신하고자하는 수신자에 관계 없이 수신된 모든 데이터를 읽을 것이다.
  장비는 macPromiscuousMode 속성을 TRUE로 설정함으로서 비규칙적인 작동 모드로 전환 할 수 있다.
  
  

# MAC 층의 책임 요약

MAC는 다음과 같은 업무를 수행한다.
● 비콘 생성(기기 조정자일 경우)
● 비콘에 기기 동기화(비콘 사용 네트워크)
● 채널 접속에 CSMA-CA 활용
● GTS 채널 접속 관리
● 2개의 동위 MAC 엔티티(두 개의 다른 장치) 간에 신뢰할 수 있는 링크 제공
● PAN 연합 및 해체 서비스 제공
● 보안 지원; MAC 계층은 자체 보안 처리를 담당하지만, 상위 계층은 어느 보안 수준을 사용할지를 결정한다.
